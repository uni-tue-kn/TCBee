...
pub unsafe fn kernel_read_tuple_from_sk(sk_ptr: *const sock, sport: u16, dport: u16) -> IpTuple {
    let family: u16 = bpf_probe_read_kernel(addr_of!((*sk_ptr).__sk_common.skc_family)).unwrap_or(0);

    let mut src_ip = [0u8; 16];
    let mut dst_ip = [0u8; 16];

    if family == AF_INET6 {
        src_ip = bpf_probe_read_kernel(addr_of!((*sk_ptr).__sk_common.skc_v6_rcv_saddr.in6_u.u6_addr8)).unwrap_or([0u8; 16]);
        dst_ip = bpf_probe_read_kernel(addr_of!((*sk_ptr).__sk_common.skc_v6_daddr.in6_u.u6_addr8)).unwrap_or([0u8; 16]);
    } else {
        let daddr = bpf_probe_read_kernel(addr_of!((*sk_ptr).__sk_common.__bindgen_anon_1.__bindgen_anon_1.skc_daddr)).unwrap_or(0);
        let saddr = bpf_probe_read_kernel(addr_of!((*sk_ptr).__sk_common.__bindgen_anon_1.__bindgen_anon_1.skc_rcv_saddr)).unwrap_or(0);
