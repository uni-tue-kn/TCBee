...
use tcbee_common::bindings::{
    flow::IpTuple,
    tcp_sock::{cwnd_trace_entry, sk_buff, sock, sock_trace_entry, tcp_sock},
};
...
fn read_kernel<T: Default>(src: *const T) -> T {
    // TODO: Check if default value causes problems
    unsafe {
...
pub fn try_tcp_recv_socket(ctx: FEntryContext) -> Result<u32, u32> {
    ...
    unsafe {
        ...
        let addr_v4 =
            read_kernel(&(*sk_ptr).__sk_common.__bindgen_anon_1.skc_addrpair).rotate_right(32);
        let ports = ports.rotate_right(16);
