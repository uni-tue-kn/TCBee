...
/// This crate has a runtime dependency on artifacts produced by the `tcbee-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "tcbee-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `tcbee-ebpf`, so cargo will automatically rebuild us
        // if `tcbee-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `tcbee-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("tcbee-ebpf");
