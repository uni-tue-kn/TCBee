use aya::{Btf, Ebpf, maps::RingBuf, programs::{FEntry, KProbe}};
...
impl BBRTracer {
    pub fn spawn(
        ...
        let congestion_hook: &mut KProbe = ebpf.program_mut("bbr_cwnd_event").unwrap().try_into()?;
        congestion_hook.load()?;
        congestion_hook.attach("bbr_cwnd_event", 0)?;

        // Both programs write to the same map
        let map =
            ebpf.take_map("BBR_EVENTS")
        ...
        writer.register::<bbr_trace_entry>(buff, prepend_string(bbr_trace_entry::FILE.to_string(), &dir))?;
