...
use ts_storage::{
    database_factory, DBBackend, DataPoint, DataValue, Flow, TimeSeries, TSDBInterface,
};
...
impl Default for DbBackend {
    fn default() -> Self {
        Self { interface: None, path: None, source: None }
    }
}
...
impl DbBackend {
    pub fn open(path: PathBuf) -> Result<Self, String> {
        ...
        Ok(Self { interface: Some(interface), path: Some(path), source: Some(source) })
    }
    ...
    pub fn list_flows(&self) -> Vec<Flow> {
        let Some(db) = &self.interface else { return Vec::new() };
        db.list_flows().ok().map(|it| it.collect()).unwrap_or_default()
    }

    pub fn list_series_for_flow(&self, flow: &Flow) -> Vec<TimeSeries> {
        let Some(db) = &self.interface else { return Vec::new() };
        db.list_time_series(flow).ok().map(|it| it.collect()).unwrap_or_default()
    }
    ...
    pub fn get_series_by_id(&self, id: i64) -> Option<TimeSeries> {
        self.interface.as_ref()?.get_time_series_by_id(id).ok().flatten()
    }
    ...
    pub fn get_series_y_bounds(&self, series_ids: &[i64]) -> Option<(f64, f64)> {
        ...
        if y_min > y_max { None } else { Some((y_min, y_max)) }
    }
    ...
    pub fn load_range(&self, series_id: i64, t_min: f64, t_max: f64) -> Vec<(f64, f64)> {
        ...
        iter.filter_map(|p| datavalue_as_f64(&p.value).map(|v| (p.timestamp, v))).collect()
    }

    /// Load string-type data points for a series in a time range.
    pub fn load_range_strings(
        &self,
        series_id: i64,
        t_min: f64,
        t_max: f64,
        ...
        iter.filter_map(|p| {
            if let DataValue::String(s) = p.value { Some((p.timestamp, s)) } else { None }
        })
        .collect()
    }
    ...
    pub fn create_series_for_flow(
        ...
        let points: Vec<DataPoint> = series
            .raw_data
            .iter()
            .map(|(t, v)| DataPoint { timestamp: *t, value: v.clone() })
            .collect();
    ...
    pub fn get_point_count(&self, series_id: i64) -> i64 {
        ...
        let Some(series) = db.get_time_series_by_id(series_id).ok().flatten() else { return 0 };
    ...
    pub fn find_series_ids_by_name(
        ...
        if found.len() != names.len() {
            Err(format!("Could not find all required series. Found {} of {}", found.len(), names.len()))
        } else {
            Ok(found)
        }
    }
